...
async function main(){
  ...
  try{
    const res = await unifiedBuyAndSellBatch(walletAdapter, tokenMint, buySol, undefined, { simulateOnly: true, debugDumpSellSwapResp: true });
    console.log('SELL swap response dump:');
    console.log(JSON.stringify(res, null, 2));
  }catch(e){ console.error('Error dumping sellSwapResp:', e); process.exit(2); }
}
